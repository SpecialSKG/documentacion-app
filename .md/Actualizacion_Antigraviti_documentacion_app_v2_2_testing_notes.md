...
## 8) Notas de testing (flujo completo Paso 1 → Paso 3) y criterios de corrección

Esta sección resume lo que se observó en una prueba real del flujo (creación de documento, armado de jerarquía, agregado por catálogo, edición de ítem, revisión en resumen y exportación).

### 8.1 Alertas/confirmaciones con UI consistente (evitar `alert()` / `confirm()`)

**Hallazgo:** varias confirmaciones y avisos se sienten “de JavaScript tradicional” (por ejemplo, confirmación de eliminar). Esto rompe el look & feel del proyecto.

**Acción recomendada (opción preferida):**
- Usar componentes de diálogo del ecosistema UI (p. ej. `AlertDialog` / `Dialog`) + toasts (ya existe `sonner` en algunos puntos).
- Mantener siempre el mismo patrón: confirmar acciones destructivas + toast al éxito/error.

**Alternativa válida:**
- Integrar **SweetAlert2** (o equivalente) y estilizarlo con la identidad del proyecto.

**Criterio de aceptación (QA):**
- Ningún flujo debe depender de `window.alert()` / `window.confirm()`.
- Las acciones destructivas (eliminar categoría/subcategoría/ítem/campo) deben pedir confirmación visual consistente.

---

### 8.2 Paso 1 (Datos Generales): validaciones configurables + selects “creatable” en TODOS los selects

#### (A) Campos obligatorios (configurable)
**Decisión actual:** el listado exacto de campos obligatorios se definirá después (se sacará un listado formal).  
**Requisito del proyecto:** dejar preparada la infraestructura para marcar campos como obligatorios por configuración (Paso 1 y Paso 2), y bloquear “Siguiente” cuando falten.

**Criterio de aceptación (QA):**
- Debe existir un lugar claro y único para declarar qué campos son obligatorios por paso.
- “Siguiente” no avanza si faltan requeridos, y el usuario ve feedback claro (sin alerts nativas).

#### (B) Todos los selects deben permitir escritura (Creatable)
**Decisión:** aplicar “creatable” **a todos los selects** (listas quemadas), porque siempre existirá el caso donde el digitador necesite una opción que no está en la lista.

**Criterio de aceptación (QA):**
- En cualquier select, el usuario puede: buscar, seleccionar, y **escribir para crear** una nueva opción.
- La opción creada debe conservarse al menos dentro del documento actual (estado local).  
  *(Opcional futuro: persistir al catálogo central.)*

---

### 8.3 Paso 2 (Detalle): Aprobadores en 2 niveles (Subcategoría e Ítem) con regla de herencia/override

#### (A) Nivel Subcategoría
**Requisito UI:** el acceso a Aprobadores debe ser explícito/visible al nivel de subcategoría, idealmente al mismo nivel visual del botón **“Agregar ítem”** cuando la subcategoría está desplegada.  
**Problema actual:** el icono “editar” de subcategoría está actuando como “editar aprobadores”, lo cual confunde porque “editar” debería permitir editar la subcategoría (nombre, mover, etc.).

#### (B) Nivel Ítem
**Requisito funcional:** también debe poder asignarse Aprobadores a nivel de Ítem, porque no todos los ítems de una subcategoría comparten aprobación.

#### (C) Regla de negocio (lo acordado)
- Si la **subcategoría** tiene aprobadores definidos, entonces **todos los ítems** de esa subcategoría usan esos aprobadores.
- Si la subcategoría **NO** tiene aprobadores, pero un **ítem** sí tiene aprobadores, entonces **solo ese ítem** usa aprobadores.
- Los ítems “hermanos” **NO** comparten aprobadores de otro ítem, salvo que el usuario los agregue manualmente allí.

**Recomendación de implementación:**
- Modelo “herencia con override”:
  - `subcategoria.aprobadores` (lista)
  - `item.aprobadores` (lista)
  - Al exportar/mostrar “efectivo”: `aprobadoresEfectivos = item.aprobadores.length ? item.aprobadores : subcategoria.aprobadores`
- UI: en el modal del ítem, mostrar un switch “Usar aprobadores propios” o “Heredar de subcategoría” (si hay aprobadores en subcategoría).

**Criterio de aceptación (QA):**
- Se puede definir aprobadores en ambos niveles.
- Se cumple la regla de herencia/override descrita arriba.
- En el resumen (Paso 3) se visualiza correctamente de dónde viene el aprobador (subcategoría vs ítem), o al menos que el comportamiento sea correcto al exportar.

---

### 8.4 Paso 2: Botón de “Editar” en subcategoría debe editar subcategoría, no solo aprobadores

**Hallazgo:** el botón/acción de editar a nivel de subcategoría no está editando “la subcategoría”, sino aprobadores.

**Acción recomendada:**
- Separar acciones:
  1) **Editar subcategoría** (nombre, mover, etc.)
  2) **Gestionar aprobadores** (subcategoría)

**Criterio de aceptación (QA):**
- El usuario distingue claramente editar subcategoría vs editar aprobadores.
- No hay ambigüedad por un único icono haciendo dos cosas conceptualmente distintas.

---

### 8.5 “Agregar desde Catálogo” (modal): listado completo + creatable + corrección de categorías incompletas

**Hallazgo:** en el modal de “Agregar desde Catálogo”, la lista de categorías mostrada se limita a 3 (Infraestructura, Aplicaciones, Seguridad), aunque el catálogo real contiene más.

**Acciones:**
- Asegurar que el modal consuma el **catálogo maestro** desde `src/data` (no solo lo ya agregado en el documento).
- Aplicar “creatable” a los 3 selects (Categoría, Subcategoría, Artículo/Ítem).
- Si el usuario escribe un valor que no existe, debe poder agregarse como opción válida (al menos dentro del documento).

**Criterio de aceptación (QA):**
- El modal lista todas las categorías reales del catálogo.
- La cascada categoría→subcategoría→ítem filtra bien.
- Se puede escribir para crear valores en cualquiera de los 3 niveles.

---

> Nota: todo lo relativo a “catálogo maestro” (categorías/subcategorías/ítems) sigue viviendo en `src/data/` (tal como se indicó en el documento de actualización), para que los cambios de data se hagan en un solo lugar y no queden “quemados” dentro de componentes.
